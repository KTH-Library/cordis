...
data <- setNames(files_csv, tbls_csv)
data$h2020topicKeywords <- files_xl
data$fp7subprogrammes <- fp7subs
...
rss_h2020 <-
  ...
  mutate(tbl = gsub(x = filename, "\\..{3}|-csv|cordis-", "")) |>
  select(tbl, filename, url) |>
  mutate(destfile = file.path(cachedir, filename)) |>
  mutate(is_zip = grepl("\\.zip$", destfile))
...
"http://digital-agenda-data.eu/download/digital-agenda-scoreboard-key-indicators.csv.zip" %>%
  download.file(file.path(cachedir, "scoreboard.csv.zip"))

scoreboard <- readr::read_csv(
  unz(ft2, filename = unzip(file.path(cachedir, "scoreboard.csv.zip"), list = TRUE)$Name),
  col_types = "ccccccccc") |>
  as_tibble()
...
dump_parquet <- function(dir = "temp") {
  ...
  con <- dbConnect(duckdb(dbdir = dbpath))
  on.exit(dbDisconnect(con, shutdown = TRUE))
  stopifnot(dir.exists(file.path(cachedir, dir)))
